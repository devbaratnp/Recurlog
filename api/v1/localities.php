...
switch ($method) {
    case 'GET':
        if ($id) {
            $row = fetchSingle('fscrm_localities', $id);
            requireExists($row, 'Locality');
            jsonResponse(toCamel($row));
        }
        [$page, $perPage, $offset] = getPageParams();
        $search = $_GET['search'] ?? '';
        [$searchClause, $searchParams] = buildSearchClause($search, ['name']);
        $stmt = $db->prepare("SELECT COUNT(*) as cnt FROM fscrm_localities WHERE 1=1 $searchClause");
        if ($searchParams) $stmt->bind_param(str_repeat('s', count($searchParams)), ...$searchParams);
        $stmt->execute();
        $total = (int)$stmt->get_result()->fetch_assoc()['cnt'];
        $stmt = $db->prepare("SELECT * FROM fscrm_localities WHERE 1=1 $searchClause ORDER BY name ASC LIMIT ? OFFSET ?");
    ...
    case 'POST':
        $input = getJsonInput();
        $data = toSnake($input);
        $row = insertAndFetch('fscrm_localities',
            ['name'],
            's',
            [$data['name'] ?? '']
        );
    ...
    case 'PUT':
        ...
        $row = updateAndFetch('fscrm_localities', ['name'], 's', [$data['name']], $id);
        requireExists($row, 'Locality');
        jsonResponse(toCamel($row));
        break;
    ...
    case 'DELETE':
        if (!$id) jsonError('ID is required', 400, 'VALIDATION_ERROR');
        requireExists(fetchSingle('fscrm_localities', $id), 'Locality');
        deleteById('fscrm_localities', $id);
