...
switch ($method) {
    case 'GET':
        if ($id) {
            $row = fetchSingle('fscrm_service_types', $id);
            requireExists($row, 'Service type');
            jsonResponse(toCamel($row));
        }
        [$page, $perPage, $offset] = getPageParams();
        $search = $_GET['search'] ?? '';
        [$searchClause, $searchParams] = buildSearchClause($search, ['name']);
        $stmt = $db->prepare("SELECT COUNT(*) as cnt FROM fscrm_service_types WHERE 1=1 $searchClause");
        if ($searchParams) $stmt->bind_param(str_repeat('s', count($searchParams)), ...$searchParams);
        $stmt->execute();
        $total = (int)$stmt->get_result()->fetch_assoc()['cnt'];
        $stmt = $db->prepare("SELECT * FROM fscrm_service_types WHERE 1=1 $searchClause ORDER BY name ASC LIMIT ? OFFSET ?");
    ...
    case 'POST':
        $input = getJsonInput();
        $data = toSnake($input);
        $row = insertAndFetch('fscrm_service_types',
            ['name'],
            's',
            [$data['name'] ?? '']
        );
    ...
    case 'PUT':
        ...
        $row = updateAndFetch('fscrm_service_types', ['name'], 's', [$data['name']], $id);
        requireExists($row, 'Service type');
        jsonResponse(toCamel($row));
        break;
    ...
    case 'DELETE':
        if (!$id) jsonError('ID is required', 400, 'VALIDATION_ERROR');
        requireExists(fetchSingle('fscrm_service_types', $id), 'Service type');
        deleteById('fscrm_service_types', $id);
        jsonResponse(['message' => 'Service type deleted']);
        break;
}
