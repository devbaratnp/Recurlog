...
Simple authentication via localStorage flag (`fscrm_auth`). Pre-filled demo credentials. Left panel shows brand identity with sidebar stats. Google sign-in is a UI placeholder. Auto-redirects to dashboard on successful login.
...
### Add Customer (`pages/customer-add.html`)

Form with:
- Name, Address, Contact Number (prefixed with +977-)
...
- On save, auto-generates a notification: "New customer [name] registered"

### Customer Detail (`pages/customer-detail.html`)

- Customer information card (name, phone, address, service chips)
- **Services table** — title, category name, type (recurring/one-time badge), next due date, status from latest task, assigned staff
- **Tasks table** — all customer tasks sorted by date descending with status pill
- "Add Service" button linking to service-add page
- Routes via `fscrm_currentCustomerId` in localStorage

### Add Service (`pages/service-add.html`)

The most feature-rich form:
- **Customer dropdown** — includes "+ Add New Customer" option at top (redirects to customer-add)
...
- **Recurrence section** (shown when Recurring selected):
  - Repeat Every: number input + unit selector (Days/Weeks/Months/Years)
  - Repeat From: radio buttons (Last Done Date | Fixed Schedule)
  - Live preview text: "This service will repeat every X days from the Last Done Date"
- First Scheduled Date picker (defaults to today)
- Staff assignment dropdown
- Notes textarea
- Validation guards all required fields
- On save, calls `saveService()` which creates the first task and a notification

### Tasks (`pages/tasks.html`)

Hash-routed tabs (`#today`, `#upcoming`, `#missed`):
- **Today** — tasks matching current date, pending/completed/missed
- **Upcoming** — future pending tasks (scheduledDate > today, status=pending)
...
- Notes field
- On confirm, calls `completeTask()` which:
  1. Marks task completed with date/notes
  2. If service is recurring, computes next due date and creates a new pending task
  3. Generates notification about completion + next scheduled date
  4. Shows success toast

Search input filters across title, customer name, and staff name.

Empty states per tab: "No today/upcoming/missed tasks" with contextual messages and "Add Service" CTA.

### Staff (`pages/staff.html`)

Card grid showing:
- Avatar (via ui-avatars.com), name, phone
- Active task count + completion rate percentage
- Progress bar
- "View Profile" link

Uses `getStaffWithStats()` which enriches staff data with computed stats.

### Staff Detail (`pages/staff-detail.html`)

- Profile card with avatar, name, phone
- **4 stat boxes** — Total Tasks, Completed, Missed, Completion Rate
- **Assigned Tasks table** — all tasks for this staff member with title, customer, date, status

### Reports (`pages/reports.html`)

Four report sections powered by Chart.js:

1. **Recurring Tasks** — all tasks from recurring services with filters (All/Missed/Today/This Month)
2. **One-Time Tasks** — all non-recurring tasks with same filters
3. **Staff-Wise** — dropdown to select staff, then shows their task stats + chart
4. **Category-Wise** — dropdown to select category, then shows task stats + chart

Each section has:
- Filter tabs (All / Missed / Today / This Month)
- Stats cards (Total, Completed %, Missed count)
- **Bar chart** (last 4 weeks of completions, built with Chart.js)
- Detail table (task title, customer, staff, date, status)

### Notifications (`pages/notifications.html`)

- Chronological list (newest first) with type-based icons and colors
- Unread items have a **brand left border + dot indicator**
...
- **Preferences** — Notifications toggle, Language selector (English/Nepali), Light/Dark theme toggle (UI only)
- **Data** — Export CSV/PDF (placeholder), **Reset Demo Data** (confirmation modal → clears all `fscrm_*` localStorage keys → reloads)
...
Point to the repository root. Framework preset: "Other". Publish directory: `.` (the root).
...
| Staff | 5 | Ramesh Yadav, Suresh Thakur, Bikash Sah, Anita Devi, Manoj Kumar — each with different completion rates (70-90%) |
